---
title: "[Sandbox] Data Simulation: New Scenario Using Melanie's Parameters [Pressure Testing]"
...
# First scenario: simple exposure effect, complex confounding effect, moderate collinearity (~ 0.25)
f_y = function(covariates){
  ...
  1 + 0.5*(covariates[, 1])^2 + 1*exp(covariates[, 2]) - exp(0.2*(covariates[, 3]*covariates[, 4])) - 0.2*(covariates[, 5])^3 + 0.5*((covariates[, 1])^2*covariates[, 5]) + 
    (2*cos(covariates[, 11]))^2 + 0.25*(covariates[, 12]*covariates[, 13]) + 0.5*(exp(covariates[, 14]))^(0.5) + 0.75*(covariates[, 15])^2
...
# Individual metals
a = linear_model_performance_dataset_level_pressure_test %>% 
...
# Mixture
b = linear_model_performance_dataset_level_pressure_test %>% 
  filter(metal == 11) %>% 
  ggplot(aes(x = model_type, y = relative_bias)) + 
  geom_boxplot() + 
  facet_grid(. ~ scenario) + 
...
# Individual metals
c = linear_model_performance_dataset_level_pressure_test %>% 
  filter(metal %in% c(1, 2, 3, 4, 5)) %>% 
...
# Mixture
d = linear_model_performance_dataset_level_pressure_test %>% 
  filter(metal == 11) %>% 
  group_by(scenario, metal, model_type) %>% 
  summarize(
    median_relative_bias = median(relative_bias)
