...
model_fit_fnct = function(model_type, data){
  ...
  if (model_type %in% c("linear")){
    ...
    SL_library_chosen = c("SL.glmnet", "SL.randomForest", "SL.ranger", "SL.xgboost", "SL.svm", "SL.earth", "SL.gbm")
...
t1 = Sys.time()
sl_df = model_fnct(scenarios_start = 10, scenarios_end = 10, metals_count = 10, boot_count = 100, model_type = "SuperLearner")
t2 = Sys.time()

# Output file
write.csv(sl_df, file = paste0("sl_estimates_scen10_", currind, ".csv"))
